...
class K_NearestNeighborRegression(object):
	...
	def  closest(self, node):
		...
		### finding the Average(Mean) of the labels- 
		### The average of the k nearest neighbor is the prediction
		sum=0
		for count in range (self.k):
			sum+=label[count]

		prediction= sum/self.k
...
###			code: 	clf=K_NearestNeighborRegression(K_Value)
